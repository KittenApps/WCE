...
export default function discreetMode(): void {
  SDK.hookFunction("CharacterSetActivePose", HOOK_PRIORITIES.Top, (args, next) => {
    if (fbcSettings.discreetMode) return null;
    return next(args);
  });

  SDK.hookFunction("PoseSetActive", HOOK_PRIORITIES.Top, (args, next) => {
    if (fbcSettings.discreetMode) return null;
    return next(args);
  });

  SDK.hookFunction("DrawImageEx", HOOK_PRIORITIES.Top, (args, next) => {
  ...
  SDK.hookFunction("ChatRoomCharacterViewDraw", HOOK_PRIORITIES.Top, (args, next) => {
    if (fbcSettings.discreetMode) {
  ...
  SDK.hookFunction("NotificationTitleUpdate", HOOK_PRIORITIES.Top, (args, next) => {
    if (fbcSettings.discreetMode) {
