...
export default function numericArousalMeters(): void {
  ...
  SDK.hookFunction("DrawArousalMeter", HOOK_PRIORITIES.Observe, (args, next) => {
    const [C] = args;
  ...
  SDK.hookFunction("DrawArousalThermometer", HOOK_PRIORITIES.Observe, (args, next) => {
    const ret = next(args);
    if (fbcSettings.numericArousalMeter && isExpanded) {
      ...
      DrawTextFit(progress.toLocaleString() + (increasing ? "↑" : " "), x + 50 * zoom, y - 30 * zoom, 100 * zoom, color, "black");
    }
    return ret;
  });
}
