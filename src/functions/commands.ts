      ...
      Action: (_, _command, [target]): void => {
        ...
        FUSAM.modals
          ...
          .then(([bindSubmit]) => {
            ...
            if (includeBinds) {
              return FUSAM.modals.openAsync({ prompt: displayText("Include locks?"), buttons: { cancel: "No", submit: "Yes" } }).then(([lockSubmit]) => {
                includeLocks = lockSubmit === "submit";
              });
            }
            return null;
          })
          .then(() => FUSAM.modals.openAsync({ prompt: displayText("Include height, body type, hair, etc?"), buttons: { cancel: "No", submit: "Yes" } }))
          .then(([baseSubmit]) => {
    ...
    },
    {
      ...
      Action: (_, command, args): void => {
        ...
        if (!target || !targetMembers || targetMembers.length === 0) {
          ...
          fbcChatNotify(
            displayText("Multiple whisper targets found: $Targets. You can still whisper the player by clicking their name or by using their member number.", {
